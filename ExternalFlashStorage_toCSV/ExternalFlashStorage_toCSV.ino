...
const int storeIntervall = 2000;
const int printIntervall = 10000;
const int statsIntervall = 20000;
...
void setup() {
  ...
  Serial.print("Create file in Flash Storage with csv header line... ");
  mbed::File file;
  auto errOpen = file.open(&fs, filename, O_WRONLY | O_CREAT | O_APPEND);
  if (errOpen) {
    Serial.print("Error opening file for writing: ");
    Serial.println(errOpen);
    //what to do when file doesn't open?
  } else {
    String csvHeaderLine = "timestamp,accelX,accelY,accelZ,temperature";

    auto errWrite = file.write(csvHeaderLine.c_str(), csvHeaderLine.length());
    if (errWrite != csvHeaderLine.length()) {
      Serial.print("Error writing csvHeaderLine: ");
      Serial.println(errWrite);
    }
    file.close();
    Serial.println(" done.");
  }
